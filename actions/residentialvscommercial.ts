import { RandCChartDataTypeMonthly, RandCChartDataTypeQuaterly, RandCChartDataTypeYearly } from "@/components/ResidentialVsCommercial/ResidentialVsCommercial";
import { ResidentialVsCommercialType } from "@/transcation/types";

export class ResvsCo {
    getYearlyData({ data }: { data: ResidentialVsCommercialType }): RandCChartDataTypeYearly {
        const result = {
            Residential: 0,
            Commercial: 0
        };
    ...
    getQuarterlyData({ data }: { data: ResidentialVsCommercialType }): RandCChartDataTypeQuaterly {
        const result = {
            Residential: 0,
            Commercial: 0
        };
    ...
    getMonthlyData({ data }: { data: ResidentialVsCommercialType }): RandCChartDataTypeMonthly {
        const result = {
            Residential: 0,
            Commercial: 0
        };
