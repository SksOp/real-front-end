...
} from "@/components/insights/bedrooms/bedrooms";
import { BedroomType } from "@/transcation/types";

export class Bedroom {
  getYearlyBedroomData({
    data,
  }: {
    data: BedroomType;
  }): BedroomChartDataTypeYearly[] {
    ...
    const quarters = Object.keys(data).reduce((acc, year) => {
      ...
      for (const month in yearData) {
        ...
        Object.keys(yearData[month]).forEach((bedrooms) => {
          ...
          acc[quarter][bedrooms] =
            (acc[quarter][bedrooms] || 0) + propertyCount;
        });
      }
      return acc;
    ...
    last12Quarters.forEach((quarter) => {
      ...
      Object.keys(quarterData).forEach((bedrooms) => {
        bedroomCount[bedrooms] =
          (bedroomCount[bedrooms] || 0) + quarterData[bedrooms];
      });
    });
  ...
  getMonthlyBedroomData({
    ...
    last12Months.forEach((month) => {
      ...
      for (const bedrooms in monthData) {
        bedroomCount[bedrooms] =
          (bedroomCount[bedrooms] || 0) + monthData[bedrooms]?.property_count ??
          0;
      }
    });
